...
describe('API Integration', () => {
  ...
  it('POST /orders - deux commandes ont deux IDs différents', async () => {
    ...
    const post2 = await request(url).post('/orders').send(makeOrderBody({ items: [{ name: 'Burger', price: 10, quantity: 2 }] }));
    expect(post1.body.id).not.toBe(post2.body.id);
  });
  it('POST /orders - commande invalide (panier vide)', async () => {
    const address = server.address();
    const url = `http://localhost:${address.port}`;
    const res = await request(url).post('/orders').send({ items: [], distance: 3, weight: 1, hour: 12, dayOfWeek: 2 });
    expect(res.status).toBe(400);
  });
  it('POST /orders - commande invalide n\'est pas enregistrée', async () => {
    const address = server.address();
    const url = `http://localhost:${address.port}`;
    await request(url).post('/orders').send({ items: [], distance: 3, weight: 1, hour: 12, dayOfWeek: 2 });
